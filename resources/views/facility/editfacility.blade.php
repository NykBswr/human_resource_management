...
<section class="w-full h-screen py-10 px-8 flex items-center justify-center">
    <div class="h-full w-6/12 bg-tertiary py-8 px-8 rounded-2xl flex flex-col items-center">
        ...
        <div class="bg-secondary w-10/12 h-full flex flex-col items-center overflow-x-auto p-20 rounded-lg justify-center">
            <div class="w-full">
                <label class="text-primary">Facility</label>
                <input type="text" class="bg-tertiary rounded-md text-primary flex flex-start gap-3 mt-4 w-full px-6 py-3 placeholder:text-primary" placeholder="{{ $facilities->facility_name }}" disabled/>
            </div>
            <form method="post" action="/facility/update/{{ $facilities->facility_id }}" class="w-full" enctype="multipart/form-data">
                @csrf
                <div class="w-full mt-4">
                    <label class="text-primary" for="description">Facility Description</label>
                    <input type="text" class="bg-tertiary rounded-md text-primary flex flex-start gap-3 mt-4 w-full px-6 py-3 placeholder:text-primary" placeholder="@if($facilities->description == null && $facilities->description == '') - @else{{ $facilities->description }}@endif" name="description" value="{{ old ('description') }}"/>
                    @error('description')
                ...
                <div class="w-ful mt-4">
                    ...
                    <input type="text" class="bg-tertiary rounded-md text-primary flex flex-start gap-3 mt-4 w-full px-6 py-3 placeholder:text-primary" placeholder="{{ $facilities->remain }}" name="remain" value="{{ old ('remain') }}"/>
                    @error('remain')
                        <span class="text-red-500">{{ $message }}</span>
                    @enderror
                </div>
